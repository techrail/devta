...
import { asciiToHex, hexToAscii, sampleInput, hexValidator } from "../../components/utils/asciiHex.js";
...
const converterOptions = ["hex", "ascii"];
...
const convertToHex = () => {
  try {
    formattedVal.value = asciiToHex(input.value)
  } catch (error) {
    console.log(error);
  }
};
...
const convertToAscii = () => {
  try {
    if (hexValidator(input.value)) {
      console.log(hexToAscii(input.value))
      formattedVal.value = hexToAscii(input.value)
    } else {
      formattedVal.value = 'invalid hex value'
    }
  } catch (error) {
    console.log(error);
  }
