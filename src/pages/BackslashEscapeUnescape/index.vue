...
import { unescapeSpecialCharacters, escapeSpecialCharacters, sampleInput, ifNull} from "../../components/utils/backslashEscapeUnescape.js";
...
const converterOptions = ["unescape", "escape"];
...
const escape = () => {
  try {
    if (ifNull(input.value)) {
      ...
      formattedVal.value = escapeSpecialCharacters(input.value)
    }
  } catch (error) {
    console.log(error);
  }
