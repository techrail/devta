          ...
          <div class="p-2">
            ...
            <div class="p-2">
              ...
              <div class="d-flex inner_input_group mt-1">
                ...
                <div class="form-floating">
                  <select class="form-select" name="timezone-select" id="timezone-select" v-model="params.dialect">
                    <option :value="dialectOptions[0]">{{ dialectOptions[0] }}</option>
                    <option v-for="zone in dialectOptions.slice(1)" :value="zone" :key="zone">{{ zone }}</option>
                  </select>
                  <label for="timezone-select">Language</label>
                </div>
                <!-- case selector -->
                <div class="form-floating">
                  <select class="form-select" name="timezone-select" id="timezone-select" v-model="params.keywordCase">
                    <option :value="keywordCaseOptions[0]">{{ keywordCaseOptions[0] }}</option>
                    <option v-for="zone in keywordCaseOptions.slice(1)" :value="zone" :key="zone">{{ zone }}</option>
                  </select>
                  <label for="timezone-select">Case</label>
                </div>
                <!-- indent style selector -->
                <div class="form-floating">
                  <select class="form-select" name="timezone-select" id="timezone-select" v-model="params.indentStyle">
                    <option :value="indentStyleOptions[0]">{{ indentStyleOptions[0] }}</option>
                    <option v-for="zone in indentStyleOptions.slice(1)" :value="zone" :key="zone">{{ zone }}</option>
                  </select>
      ...
      <div class="block card block2">
        <div class="d-flex flex-column h-100 justify-content-between">
          <div class="p-2 overflow-auto">
            <div v-if="error">
              <div class="alert alert-danger" role="alert">Invalid SQL</div>
            </div>
